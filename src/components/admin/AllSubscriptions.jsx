...
import { SubscriptionsInput } from './AllSubscriptions/SubscriptionsInput';
...
export const AllSubscriptions = () => {
  ...
  const addNewSubscription = (newSubsciptionData) => {
    axios.post(`${baseURL}/subscriptions`,newSubsciptionData)
    .then((res) => {
  ...
  const deleteSubscription = (id) => {
    axios.delete(`${baseURL}/subscriptions/${id}`)
    .then((res) => {
      console.log(res);
      getSubscriptionsData();
      // setSubscriptions
  ...
  const AddNewSubscriptionForm = () => {
    ...
    return (
      <NEWDIV>
        ...
        <SubscriptionsInput subscriptionData={subscriptionData} handleChange={handleChange} />
  ...
  return (
    <div>
      { 
        !newState && <div style={{width:"200px", display:"flex", justifyContent:"flex-start", marginBottom:"10px"}}>
          <Button children={"Add Subscription"} onClick={handleAddSubscription} />
        </div>
      }
      {newState && <AddNewSubscriptionForm />}
      <DIV className="all-subscriptions-container">
        { !newState && subscriptions && subscriptions.map((ele) => {
          return(
          <div key={ele.id} className='subscriptions-card'>
            ...
            <div className='subscriptions-card-buttons-div'>
              ...
              <ButtonSmall children="Delete" onClick={() => {handleDelete(ele.id)}} />
            </div>
          </div>
          ) 
        })}
